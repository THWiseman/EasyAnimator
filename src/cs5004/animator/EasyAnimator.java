...
/**
 * This class holds the main() method for the EasyAnimator application. Main can take the following
 * arguments in any order: -in [filepath]: This argument is mandatory and tells the program which
 * text file to use in order to display the animation. -view [viewtype]: This argument is mandatory
 * and tells the program which view to display. Currently 'text', 'svg', and 'visual' are the
 * supported options. -out [filepath]: This argument is optional and tells the program what to call
 * the output file is outputs (and where to put it). This argument is ignored for some views. If
 * text output is a requirement of a view but no output parameter is provided, System.out will be
 * used by default. -speed [int]: This argument is optional and tells the program how many frams per
 * second the animation should be displayed ay. Any integer one or greater is allowed. Example:
 * ./EasyAnimator.jar -in src/toh-12.txt -view svg -speed 100 -out output.svg
 */
